...
object LexicalParser {
  ...
  private def token(key: String, value: String): Either[String, Token] = key match {
  ...
  def parse(data: String): LexicalParserResult = {
    val results = data.split("\n").toList.zipWithIndex.
      ...
      map{ case (value, i) =>  parseLine(value).leftMap(error => s"Line ${i+1}: ${error}") }
